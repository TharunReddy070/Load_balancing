...
def simulate_traffic(client, load_balancer, num_requests=1000):
    """
...
def visualize_metrics(metrics):
    try:
        ...
        # Plot 3: Throughput Distribution (Line Plot only)
        throughputs = metrics['throughputs']
        # Calculate the frequency of each throughput in the bins
        n, bins = np.histogram(throughputs, bins=20)
        bin_centers = 0.5 * (bins[1:] + bins[:-1])  # Find the center of each bin

        # Plot a line using bin centers and the frequencies (No bars)
        ax3.plot(bin_centers, n, 'o-', color='blue')  # Blue line with markers
        ax3.set_title('Throughput Distribution (Line Plot)')
